    ...
    for configuration in configurations:
        if configuration in model_mapping and configuration in tokenizer_mapping:
            ...
            if tokenizer is not None:
                if configuration.__name__.startswith(
                    tokenizer.__name__.replace("Tokenizer", "")
                ):
                    model_tokenizer_mapping.update({tokenizer: (configuration, model)})
            if tokenizer_fast is not None:
                if configuration.__name__.startswith(
                    tokenizer_fast.__name__.replace("TokenizerFast", "")
                ):
...
class TokenizerUtilTester(unittest.TestCase):
    ...
    def test_legacy_load_from_one_file(self):
        ...
        try:
            ...
            with open(tmp_file, "wb") as f:
                http_get(
                    "https://huggingface.co/albert-base-v1/resolve/main/spiece.model", f
                )
    ...
    def test_legacy_load_from_url(self):
        # This test is for deprecated behavior and can be removed in v5
        _ = AlbertTokenizer.from_pretrained(
