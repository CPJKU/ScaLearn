...
class TFLongformerModelIntegrationTest(unittest.TestCase):
    ...
    def test_mask_invalid_locations(self):
        ...
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_1), tf.int64)) == 8
        )
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_2), tf.int64)) == 24
        )
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_3), tf.int64)) == 24
        )
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_4), tf.int64)) == 12
        )
    ...
    def test_chunk(self):
        ...
        expected_slice_along_seq_length = tf.convert_to_tensor(
            [0.4983, -0.7584, -1.6944], dtype=tf.float32
        )
        expected_slice_along_chunk = tf.convert_to_tensor(
            [0.4983, -1.8348, -0.7584, 2.0514], dtype=tf.float32
        )
    ...
    def test_layer_local_attn(self):
        ...
        tf.debugging.assert_near(
            output_hidden_states[0, 1], expected_slice, rtol=1e-3, atol=1e-4
        )
    ...
    def test_layer_global_attn(self):
        ...
        tf.debugging.assert_near(
            output_hidden_states[0, 2], expected_slice_0, rtol=1e-3, atol=1e-4
        )
        tf.debugging.assert_near(
            output_hidden_states[1, -2], expected_slice_1, rtol=1e-3, atol=1e-4
        )
    ...
    def test_inference_no_head(self):
        ...
        input_ids = tf.convert_to_tensor(
            [[0, 20920, 232, 328, 1437, 2]], dtype=tf.int64
        )
        ...
        tf.debugging.assert_near(
            output[0, 0, -5:], expected_output_slice, rtol=1e-3, atol=1e-4
        )
        tf.debugging.assert_near(
            output_without_mask[0, 0, -5:], expected_output_slice, rtol=1e-3, atol=1e-4
        )
    ...
    def test_inference_no_head_long(self):
        ...
        input_ids = tf.convert_to_tensor(
            [[0] + [20920, 232, 328, 1437] * 1000 + [2]], dtype=tf.int64
        )
        ...
        # assert close
        tf.debugging.assert_near(
            tf.reduce_sum(output), expected_output_sum, rtol=1e-4, atol=1e-4
        )
        tf.debugging.assert_near(
            tf.reduce_mean(output), expected_output_mean, rtol=1e-4, atol=1e-4
        )
    ...
    def test_inference_masked_lm_long(self):
        ...
        # assert close
        tf.debugging.assert_near(
            tf.reduce_mean(loss), expected_loss, rtol=1e-4, atol=1e-4
        )
        tf.debugging.assert_near(
            tf.reduce_sum(prediction_scores),
