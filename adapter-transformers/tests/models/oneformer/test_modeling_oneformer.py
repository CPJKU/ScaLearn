...
class OneFormerModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        task_inputs = (
            torch.randint(high=49408, size=(self.batch_size, self.sequence_length))
            .to(torch_device)
            .long()
        )
    ...
    def test_model_with_labels(self):
        ...
        inputs = {
            ...
            "task_inputs": torch.randint(
                high=49408, size=(2, 77), device=torch_device
            ).long(),
            "text_inputs": torch.randint(
                high=49408, size=(2, 134, 77), device=torch_device
            ).long(),
...
class OneFormerModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice_hidden_state = torch.tensor(
            [
                [0.2724, 0.8287, 0.6025],
                [1.2706, 1.1252, 1.1445],
                [1.1357, 0.6150, 0.4185],
            ]
        ).to(torch_device)
        self.assertTrue(
            torch.allclose(
        ...
        expected_slice_hidden_state = torch.tensor(
            [
                [1.0581, 1.2275, 1.2000],
                [1.1901, 1.2925, 1.2861],
                [1.1578, 1.2558, 1.3212],
            ]
        ).to(torch_device)
        self.assertTrue(
            torch.allclose(
        ...
        expected_slice_hidden_state = torch.tensor(
            [
                [3.0711, -1.1855, -5.1095],
                [3.5536, -3.2710, -5.2052],
                [2.6020, -4.3605, -4.1422],
            ]
        ).to(torch_device)
        self.assertTrue(
            torch.allclose(
    ...
    def test_inference_universal_segmentation_head(self):
        ...
        expected_slice = [
            [
                [3.1215, 4.1250, 4.1106],
                [2.8183, 3.4623, 3.5512],
                [2.4550, 2.9841, 3.5081],
            ]
        ]
