...
class OneFormerImageProcessingTest(ImageProcessingSavingTestMixin, unittest.TestCase):
    ...
    def test_call_pil(self):
        ...
        image_inputs = prepare_image_inputs(
            self.image_processing_tester, equal_resolution=False
        )
        for image in image_inputs:
        ...
        (
            expected_height,
            expected_width,
        ) = self.image_processing_tester.get_expected_values(image_inputs)
        ...
        (
            expected_height,
            expected_width,
        ) = self.image_processing_tester.get_expected_values(image_inputs, batched=True)
    ...
    def test_call_numpy(self):
        ...
        image_inputs = prepare_image_inputs(
            self.image_processing_tester, equal_resolution=False, numpify=True
        )
        for image in image_inputs:
    ...
    def test_equivalence_pad_and_create_pixel_mask(self):
        ...
        encoded_images = image_processor_2(
            image_inputs, ["semantic"] * len(image_inputs), return_tensors="pt"
        )
