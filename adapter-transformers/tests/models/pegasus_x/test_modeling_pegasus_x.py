...
class PegasusXModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_pegasus_x_inputs_dict(
...
class PegasusXModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (
        (PegasusXForConditionalGeneration,) if is_torch_available() else ()
    )
    ...
    def _check_encoder_hidden_states_for_generate(
        ...
        encoder_expected_shape = (
            batch_size,
            self.round_up(seq_length, config.block_size),
            config.hidden_size,
        )
...
class PegasusXModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = torch.tensor(
            [
                ...
                [0.0836, -0.1848, 0.1304],
                [0.0673, -0.1686, 0.1045],
            ],
            device=torch_device,
        )
    ...
    def test_inference_head(self):
        ...
        expected_slice = torch.tensor(
            [
                [0.0, 9.5705185, 1.5897303],
                [0.0, 9.833374, 1.5828674],
                [0.0, 10.429961, 1.5643371],
            ],
            device=torch_device,
        )
        self.assertTrue(
    ...
    def test_seq_to_seq_generation(self):
        hf = PegasusXForConditionalGeneration.from_pretrained(
...
@require_torch
class PegasusXStandaloneDecoderModelTest(
