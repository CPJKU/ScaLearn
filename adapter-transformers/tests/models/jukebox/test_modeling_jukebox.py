...
class Jukebox1bModelTester(unittest.TestCase):
    ...
    def test_sampling(self):
        ...
        zs = model._sample(
            ...
            labels,
            [0],
            sample_length=40 * model.priors[0].raw_to_tokens,
            save_results=False,
        )
        ...
        zs = model._sample(
            ...
            labels,
            [1],
            sample_length=40 * model.priors[1].raw_to_tokens,
            save_results=False,
        )
        ...
        zs = model._sample(
            ...
            labels,
            [2],
            sample_length=40 * model.priors[2].raw_to_tokens,
            save_results=False,
        )
    ...
    def test_conditioning(self):
        ...
        start = 0
        music_token_conds = top_prior.get_music_tokens_conds(
            zs, start=start, end=start + top_prior.n_ctx
        )
        ...
        audio_conditioning, metadata_conditioning, lyric_tokens = top_prior.get_cond(
            music_token_conds, metadata
        )
        torch.testing.assert_allclose(
            audio_conditioning[0][0][:30].detach(),
            torch.tensor(self.EXPECTED_AUDIO_COND),
            atol=1e-4,
            rtol=1e-4,
        )
        torch.testing.assert_allclose(
            metadata_conditioning[0][0][:30].detach(),
            torch.tensor(self.EXPECTED_META_COND),
            atol=1e-4,
            rtol=1e-4,
        )
        torch.testing.assert_allclose(
            lyric_tokens[0, :30].detach(),
            torch.tensor(self.EXPECTED_LYRIC_COND),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_primed_sampling(self):
        ...
        zs = [
            model.vqvae.encode(waveform, start_level=2, bs_chunks=waveform.shape[0])[0],
            None,
            None,
        ]
        zs = model._sample(
        ...
        torch.testing.assert_allclose(
            zs[0][0][:40], torch.tensor(self.EXPECTED_PRIMED_0)
        )

        upper_2 = torch.cat(
            (zs[0], torch.zeros(1, 2048 - zs[0].shape[-1])), dim=-1
        ).long()
        zs = [
            upper_2,
            model.vqvae.encode(waveform, start_level=1, bs_chunks=waveform.shape[0])[0],
            None,
        ]
        zs = model._sample(
            zs,
        ...
        torch.testing.assert_allclose(
            zs[1][0][:40], torch.tensor(self.EXPECTED_PRIMED_1)
        )

        upper_1 = torch.cat(
            (zs[1], torch.zeros(1, 2048 - zs[1].shape[-1])), dim=-1
        ).long()
        zs = [
            upper_2,
            upper_1,
            model.vqvae.encode(waveform, start_level=0, bs_chunks=waveform.shape[0])[0],
        ]
        zs = model._sample(
            zs,
            tokens,
        ...
        torch.testing.assert_allclose(
            zs[2][0][:40].cpu(), torch.tensor(self.EXPECTED_PRIMED_2)
        )
    ...
    def test_vqvae(self):
        ...
        torch.testing.assert_allclose(
            zs[0][0], torch.tensor(self.EXPECTED_VQVAE_ENCODE)
        )

        with torch.no_grad():
            x = model.vqvae.decode(zs, start_level=2, bs_chunks=x.shape[0])
        torch.testing.assert_allclose(
            x[0, :40, 0], torch.tensor(self.EXPECTED_VQVAE_DECODE), atol=1e-4, rtol=1e-4
        )
...
class Jukebox5bModelTester(unittest.TestCase):
    ...
    def test_sampling(self):
        ...
        zs = model._sample(
            ...
            labels,
            [0],
            sample_length=60 * model.priors[0].raw_to_tokens,
            save_results=False,
        )
        torch.testing.assert_allclose(zs[0][0], torch.tensor(self.EXPECTED_OUTPUT_2))
        ...
        zs = model._sample(
            ...
            labels,
            [1],
            sample_length=60 * model.priors[1].raw_to_tokens,
            save_results=False,
        )
        torch.testing.assert_allclose(zs[1][0], torch.tensor(self.EXPECTED_OUTPUT_1))
        ...
        zs = model._sample(
            ...
            labels,
            [2],
            sample_length=60 * model.priors[2].raw_to_tokens,
            save_results=False,
        )
        torch.testing.assert_allclose(zs[2][0], torch.tensor(self.EXPECTED_OUTPUT_0))
    ...
    def test_slow_sampling(self):
        ...
        torch.testing.assert_allclose(
            zs[0][0].cpu(), torch.tensor(self.EXPECTED_GPU_OUTPUTS_2)
        )
        model.priors[0].cpu()
        ...
        torch.testing.assert_allclose(
            zs[1][0].cpu(), torch.tensor(self.EXPECTED_GPU_OUTPUTS_1)
        )
        model.priors[1].cpu()
        ...
        torch.testing.assert_allclose(
            zs[2][0].cpu(), torch.tensor(self.EXPECTED_GPU_OUTPUTS_0)
        )
    ...
    def test_fp16_slow_sampling(self):
        ...
        model = (
            JukeboxPrior.from_pretrained(prior_id, min_duration=0)
            .eval()
            .half()
            .to("cuda")
        )
        ...
        torch.testing.assert_allclose(
            outputs[0].cpu(), torch.tensor(self.EXPECTED_GPU_OUTPUTS_2)
        )
