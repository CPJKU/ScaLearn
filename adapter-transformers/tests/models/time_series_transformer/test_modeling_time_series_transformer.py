...
class TimeSeriesTransformerModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=TimeSeriesTransformerConfig, has_text_modality=False
        )
...
def prepare_batch(filename="train-batch.pt"):
    file = hf_hub_download(
        repo_id="kashif/tourism-monthly-batch", filename=filename, repo_type="dataset"
    )
...
class TimeSeriesTransformerModelIntegrationTests(unittest.TestCase):
    def test_inference_no_head(self):
        ...
        expected_shape = torch.Size(
            (64, model.config.prediction_length, model.config.d_model)
        )
        self.assertEqual(output.shape, expected_shape)

        expected_slice = torch.tensor(
            [
                [-0.3125, -1.2884, -1.1118],
                [-0.5801, -1.4907, -0.7782],
                [0.0849, -1.6557, -0.9755],
            ],
            device=torch_device,
        )
        self.assertTrue(
    ...
    def test_inference_head(self):
        ...
        expected_slice = torch.tensor(
            [
                [0.9127, -0.2056, -0.5259],
                [1.0572, 1.4104, -0.1964],
                [0.1358, 2.0348, 0.5739],
            ],
            device=torch_device,
        )
        self.assertTrue(
