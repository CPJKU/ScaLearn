...
class BitModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=BitConfig, has_text_modality=False
        )
...
class BitModelIntegrationTest(unittest.TestCase):
    ...
    def default_feature_extractor(self):
        return (
            BitImageProcessor.from_pretrained(BIT_PRETRAINED_MODEL_ARCHIVE_LIST[0])
            if is_vision_available()
            else None
        )
    ...
    def test_inference_image_classification_head(self):
        model = BitForImageClassification.from_pretrained(
            BIT_PRETRAINED_MODEL_ARCHIVE_LIST[0]
        ).to(torch_device)
