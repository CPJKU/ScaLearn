...
class Mask2FormerImageProcessingTest(ImageProcessingSavingTestMixin, unittest.TestCase):
    ...
    def test_call_pil(self):
        ...
        (
            expected_height,
            expected_width,
        ) = self.image_processor_tester.get_expected_values(image_inputs)
        ...
        (
            expected_height,
            expected_width,
        ) = self.image_processor_tester.get_expected_values(image_inputs, batched=True)
    ...
    def test_equivalence_pad_and_create_pixel_mask(self):
        ...
        encoded_images_with_method = image_processing_1.encode_inputs(
            image_inputs, return_tensors="pt"
        )
    ...
    def test_integration_instance_segmentation(self):
        ...
        image_processing = Mask2FormerImageProcessor(
            reduce_labels=True, ignore_index=255, size=(512, 512)
        )
    ...
    def test_post_process_instance_segmentation(self):
        feature_extractor = self.image_processing_class(
            num_labels=self.image_processor_tester.num_classes
        )
        outputs = self.image_processor_tester.get_fake_mask2former_outputs()
