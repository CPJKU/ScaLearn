...
class BlipModelIntegrationTest(unittest.TestCase):
    def test_inference_image_captioning(self):
        ...
        inputs = processor(images=image, text=context, return_tensors="pt").to(
            torch_device
        )
    ...
    def test_inference_itm(self):
        ...
        self.assertTrue(
            torch.allclose(
                torch.nn.Softmax()(out_itm[0].cpu()),
                expected_scores,
                rtol=1e-3,
                atol=1e-3,
            )
        ...
        self.assertTrue(
            torch.allclose(out[0].cpu(), torch.Tensor([[0.5053]]), rtol=1e-3, atol=1e-3)
        )
