            ...
            for size in [config.vocab_size - 10, config.vocab_size + 10, None]:
                ...
                if old_bias is not None and new_bias is not None:
                    for old_weight, new_weight in zip(
                        ...
                        models_equal = True
                        for p1, p2 in zip(
                            tf.squeeze(old_weight), tf.squeeze(new_weight)
                        ):
                            if tf.math.reduce_sum(tf.math.abs(p1 - p2)) > 0:
                                models_equal = False
                        self.assertTrue(models_equal)
        ...
        for model_class in self.all_model_classes:
            ...
            if hasattr(self.model_tester, "batch_size"):
                sample_weight = tf.convert_to_tensor(
                    [0.5] * self.model_tester.batch_size, dtype=tf.float32
                )
            else:
                sample_weight = None
            ...
            if sample_weight is not None:
                ...
                weighted_dataset = dataset.map(
                    lambda x: (x, None, tf.convert_to_tensor(0.5, dtype=tf.float32))
                )
            else:
    ...
    def test_int_support(self):
        ...
        for model_class in self.all_model_classes:
            ...
            for key, tensor in model.dummy_inputs.items():
                self.assertTrue(
                    isinstance(tensor, tf.Tensor), "Dummy inputs should be tf.Tensor!"
                )
                if tensor.dtype.is_integer:
                    self.assertTrue(
                        tensor.dtype == tf.int32,
                        "Integer dummy inputs should be tf.int32!",
                    )
            ...
            if hasattr(model, "serving"):
                ...
                for key, tensor_spec in serving_sig[0].items():
                    if tensor_spec.dtype.is_integer:
                        self.assertTrue(
                            tensor_spec.dtype == tf.int32,
                            "Serving signatures should use tf.int32 for ints!",
                        )
        ...
        def _generate_and_check_results(model, inputs_dict):
            if "input_ids" in inputs_dict:
                ...
                if model.generation_config.pad_token_id is not None:
                    ...
                    new_pad_token = None
                inputs = tf.where(
                    inputs != model.generation_config.pad_token_id,
                    inputs,
                    new_pad_token,
                )
