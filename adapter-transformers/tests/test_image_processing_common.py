...
def prepare_image_inputs(
    image_processor_tester, equal_resolution=False, numpify=False, torchify=False
):
    ...
    for i in range(image_processor_tester.batch_size):
        if equal_resolution:
            ...
            width, height = np.random.choice(
                np.arange(min_resolution, image_processor_tester.max_resolution), 2
            )
        image_inputs.append(
            np.random.randint(
                255,
...
def prepare_video(
    image_processor_tester, width=10, height=10, numpify=False, torchify=False
):
...
def prepare_video_inputs(
    ...
    for i in range(image_processor_tester.batch_size):
        if equal_resolution:
            ...
            width, height = np.random.choice(
                np.arange(
                    image_processor_tester.min_resolution,
                    image_processor_tester.max_resolution,
                ),
                2,
            )
            video = prepare_video(
...
class ImageProcessorPushToHubTester(unittest.TestCase):
    ...
    def test_push_to_hub(self):
        ...
        new_image_processor = ViTImageProcessor.from_pretrained(
            f"{USER}/test-image-processor"
        )
    ...
    def test_push_to_hub_in_organization(self):
        ...
        image_processor.push_to_hub(
            "valid_org/test-image-processor", use_auth_token=self._token
        )

        new_image_processor = ViTImageProcessor.from_pretrained(
            "valid_org/test-image-processor"
        )
        for k, v in image_processor.__dict__.items():
            self.assertEqual(v, getattr(new_image_processor, k))
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            image_processor.save_pretrained(
                tmp_dir,
                repo_id="valid_org/test-image-processor-org",
                push_to_hub=True,
                use_auth_token=self._token,
            )

        new_image_processor = ViTImageProcessor.from_pretrained(
            "valid_org/test-image-processor-org"
        )
        for k, v in image_processor.__dict__.items():
            self.assertEqual(v, getattr(new_image_processor, k))
    ...
    def test_push_to_hub_dynamic_image_processor(self):
        ...
        image_processor.push_to_hub(
            "test-dynamic-image-processor", use_auth_token=self._token
        )
