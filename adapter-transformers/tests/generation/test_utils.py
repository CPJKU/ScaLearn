...
class GenerationTesterMixin:
    ...
    def test_generate_without_input_ids(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            output_ids_generate = model.generate(
                do_sample=False, max_length=max_length, remove_invalid_values=True
            )
    ...
    def test_contrastive_generate_dict_outputs_use_cache(self):
        for model_class in self.all_generative_model_classes:
            ...
            self.assertListEqual(
                output_generate.sequences.tolist(),
                output_contrastive.sequences.tolist(),
            )
...
class GenerationIntegrationTests(unittest.TestCase):
    ...
    def test_max_new_tokens_decoder_only_contrastive_search_t5(self):
        ...
        input_ids = t5_tokenizer(article, return_tensors="pt").input_ids.to(
            torch_device
        )
        ...
        outputs = t5_model.generate(
            input_ids, max_new_tokens=max_new_tokens, penalty_alpha=0.6, top_k=4
        )
        # 1 BOS + 3 new tokens
        self.assertEqual(list(outputs.shape), [1, 4])
        ...
        # Encoder decoder call > 20
        outputs = t5_model.generate(
            max_new_tokens=max_new_tokens + 20, penalty_alpha=0.6, top_k=4
        )
    ...
    def test_max_new_tokens_decoder_only_contrastive_search_gptj(self):
        ...
        gptj_tokenizer = AutoTokenizer.from_pretrained(
            "hf-internal-testing/tiny-random-gptj"
        )
        gptj_model = AutoModelForCausalLM.from_pretrained(
            "hf-internal-testing/tiny-random-gptj"
        ).to(torch_device)
        input_ids = gptj_tokenizer(article, return_tensors="pt").input_ids.to(
            torch_device
        )
