...
class TestAdapterTrainer(unittest.TestCase):
    ...
    def test_resume_training(self):
        ...
        with TemporaryDirectory() as tmpdirname:
            ...
            model_resume = AutoModelForSequenceClassification.from_config(
                self.get_model_config()
            )
            ...
            trainer_resume = AdapterTrainer(
                ...
                args=TrainingArguments(
                    do_train=True, max_steps=1, output_dir=tmpdirname
                ),
                train_dataset=train_dataset,
            )
            ...
            self.assertEqual(
                model.config.adapters.adapters, model_resume.config.adapters.adapters
            )
    ...
    def test_resume_training_with_fusion(self):
        ...
        with TemporaryDirectory() as tmpdirname:
            ...
            for (k1, v1), (k2, v2) in zip(
                trainer.model.to("cpu").state_dict().items(),
                trainer_resume.model.to("cpu").state_dict().items(),
            ):
                self.assertEqual(k1, k2)
                if "adapter" in k1:
                    self.assertTrue(torch.equal(v1, v2), k1)
    ...
    def test_auto_set_save_adapters(self):
        ...
        model.train_transfer_layer(Fuse("adapter1", "adapter2"))
        ...
        with TemporaryDirectory() as tmpdirname:
            ...
            self.assertTrue(trainer.train_transfer_layer)
    ...
    def test_training_load_best_model_at_end_adapter(self):
        ...
        with TemporaryDirectory() as tmpdirname:
            ...
            with self.assertLogs(logger) as cm:
                trainer.train()
                self.assertTrue(
                    any("Loading best adapter(s) from" in line for line in cm.output)
                )
    ...
    def test_training_load_best_model_at_end_fusion(self):
        ...
        model.train_transfer_layer(Fuse("fuse_adapter_1", "fuse_adapter_2"))
        ...
        with TemporaryDirectory() as tmpdirname:
            ...
            with self.assertLogs(logger) as cm:
                ...
                self.assertTrue(
                    any(
                        "Loading best adapter fusion(s) from" in line
                        for line in cm.output
                    )
                )
            self.assertEqual(
                Fuse("fuse_adapter_1", "fuse_adapter_2"), trainer.model.active_adapters
            )
    ...
    def test_reloading_prediction_head(self):
        ...
        model.train_transfer_layer(adapter_setup)
        ...
        with TemporaryDirectory() as tempdir:
            ...
            model_resume.train_transfer_layer(adapter_setup)
    ...
    def test_general(self):
        ...
        with TemporaryDirectory() as tempdir:
            ...
            files_dir_checkpoint = [
                file_or_dir
                for file_or_dir in os.listdir(os.path.join(tempdir, "checkpoint-1"))
            ]
