...
from transformers import BertConfig, BertForSequenceClassification, PfeifferConfig, PrefixTuningConfig
...
class AdapterCompositionParsingTest(unittest.TestCase):
    ...
    def test_invalid_nesting_fusion(self):
        ...
        self.assertRaises(
            ValueError, lambda: parse_composition(Fuse(Split("a", "b", 128), "c"))
        )

    def test_invalid_nesting_split(self):
        self.assertRaises(
            ValueError, lambda: parse_composition(Split("a", Fuse("b", "c"), 128))
        )
