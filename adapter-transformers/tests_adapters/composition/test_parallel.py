...
class ParallelAdapterInferenceTestMixin:
    def test_parallel_inference_with_heads(self):
        ...
        inputs["attention_mask"] = torch.randint(
            0, 2, size=(3, 64), device=torch_device
        )
...
class ParallelTrainingMixin:
    ...
    def run_parallel_training_test(self, adapter_config, filter_key):
        ...
        for k, v in filter_parameters(model, "encoder.layer.0.attention").items():
            if (
                filter_key.format("mrpc1") not in k
                and filter_key.format("mrpc2") not in k
            ):
        ...
        self.assertTrue(
            any(
                [
                    not torch.equal(v, state_dict_pre[k])
        ...
        self.assertTrue(
            all(
                torch.equal(v, state_dict_pre[k])
                for k, v in model.state_dict().items()
                if "mrpc" not in k
            )
        )
    ...
    def run_parallel_training_equivalent_to_single(self, adapter_config):
        ...
        for i in range(3):
            ...
            if isinstance(model, T5AdapterModel) or isinstance(
                model, BertGenerationAdapterModel
            ):
        ...
        for k, v in state_dict.items():
            ...
            if b1 in k:
                self.assertTrue(
                    torch.allclose(v, state_dict[k.replace(b1, b2)], atol=1e-5)
                )
