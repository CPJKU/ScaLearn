...
class PredictionHeadModelTestMixin:
    ...
    def test_classification_head(self):
        if not hasattr(
            ADAPTER_MODEL_MAPPING[self.config_class], "add_classification_head"
        ):
    ...
    def test_image_classification_head(self):
        if not hasattr(
            ADAPTER_MODEL_MAPPING[self.config_class], "add_image_classification_head"
        ):
        ...
        self.run_prediction_head_test(
            model1, model2, "dummy", input_shape=(1, 3, 224, 224), label_dict=label_dict
        )

    def test_multiple_choice_head(self):
        if not hasattr(
            ADAPTER_MODEL_MAPPING[self.config_class], "add_multiple_choice_head"
        ):
    ...
    def test_tagging_head(self):
        ...
        self.run_prediction_head_test(
            ...
            model2,
            "dummy",
            output_shape=(self.batch_size, self.seq_length, 2),
            label_dict=label_dict,
        )
    ...
    def test_qa_head(self):
        ...
        label_dict = {}
        label_dict["start_positions"] = torch.zeros(
            self.batch_size, dtype=torch.long, device=torch_device
        )
        label_dict["end_positions"] = torch.zeros(
            self.batch_size, dtype=torch.long, device=torch_device
        )
        self.run_prediction_head_test(
            model1,
            model2,
            "dummy",
            output_shape=(self.batch_size, self.seq_length),
            label_dict=label_dict,
        )
    ...
    def test_causal_lm_head(self):
        ...
        input_ids = self.get_input_samples((1, self.seq_length), config=model1.config)[
            "input_ids"
        ]
    ...
    def test_seq2seq_lm_head(self):
        ...
        if hasattr(model1, "prepare_decoder_input_ids_from_labels"):
            decoder_input_ids = model1.prepare_decoder_input_ids_from_labels(
    ...
    def test_dependency_parsing_head(self):
        if not hasattr(
            ADAPTER_MODEL_MAPPING[self.config_class], "add_dependency_parsing_head"
        ):
        ...
        self.run_prediction_head_test(
            ...
            model2,
            "dummy",
            output_shape=(1, self.seq_length, self.seq_length + 1, 2),
            label_dict=label_dict,
        )
    ...
    def test_invertible_adapter_with_head(self):
        ...
        in_data = self.get_input_samples(
            (self.batch_size, self.seq_length), config=model.config
        )
        model.to(torch_device)
        out = model(**in_data)

        self.assertEqual(
            (self.batch_size, self.seq_length, model.config.vocab_size), out[0].shape
        )
    ...
    def test_save_all_adapters_with_head(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertTrue(
                os.path.isfile(os.path.join(tmp_dir, "test", "head_config.json"))
            )
