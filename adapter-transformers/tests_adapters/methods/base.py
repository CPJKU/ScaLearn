...
class AdapterMethodBaseTestMixin:
    ...
    def filter_parameters(self, model, filter_keys):
        return {
            ...
            if any([filter_key in k for filter_key in filter_keys])
        }
    ...
    def run_get_test(self, model, adapter_config, num_expected_modules):
        ...
        num_found_modules = sum(
            [len(layer_modules) for layer_modules in adapter.values()]
        )
    ...
    def run_load_test(self, adapter_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            weights = torch.load(
                os.path.join(temp_dir, WEIGHTS_NAME), map_location="cpu"
            )
    ...
    def run_full_model_load_test(self, adapter_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            model2, loading_info = self.model_class.from_pretrained(
                temp_dir, output_loading_info=True
            )
        ...
        def has_tied_embeddings(k):
            ...
            is_tied_layer = (
                ...
                and "heads.{}.{}.weight".format(
                    "mrpc", len(model.heads["mrpc"]._modules) - 1
                )
                in k
            )
