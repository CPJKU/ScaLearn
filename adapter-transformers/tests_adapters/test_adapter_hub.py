...
from transformers import ADAPTER_CONFIG_MAP  # get_adapter_config_hash,
...
SAMPLE_INDEX = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "fixtures/hub-index.sample.json"
)
...
class AdapterHubTest(unittest.TestCase):
    ...
    def test_find_in_index(self):
        for sample in self.search_samples:
            with self.subTest(sample=sample):
                ...
                found_entry = find_in_index(
                    sample[0], None, config, index_file=SAMPLE_INDEX
                )
    ...
    def test_load_task_adapter_from_hub_with_leave_out(self):
        ...
        loading_info = {}
        adapter_name = model.load_adapter(
            "sts/mrpc@ukp", config="pfeiffer", loading_info=loading_info, leave_out=[11]
        )
        ...
        self.assertNotIn(
            adapter_name, model.base_model.encoder.layer[11].output.adapters
        )
        ...
        for config in ["pfeiffer+inv", "houlsby+inv"]:
            with self.subTest(config=config):
                ...
                config = AdapterConfig.load(
                    config, non_linearity="gelu", reduction_factor=2
                )
