...
class AdapterFusionModelTestMixin:
    def test_add_adapter_fusion(self):
        ...
        for (
            adater_fusion_config_name,
            adapter_fusion_config,
        ) in ADAPTERFUSION_CONFIG_MAP.items():
            ...
            with self.subTest(model_class=model.__class__.__name__, config=config_name):
                ...
                self.assertEqual(
                    asdict(adapter_config), asdict(model.config.adapters.get(name1))
                )
                self.assertEqual(
                    asdict(adapter_config), asdict(model.config.adapters.get(name2))
                )
        ...
        for (
            ...
            with self.subTest(model_class=model1.__class__.__name__):
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    self.assertTrue(
                        os.path.exists(
                            os.path.join(temp_dir, ADAPTERFUSION_WEIGHTS_NAME)
                        )
                    )

                # check if adapter was correctly loaded
                self.assertEqual(
                    model1.config.adapters.fusions.keys(),
                    model2.config.adapters.fusions.keys(),
                )
    ...
    def test_load_full_model_fusion(self):
        ...
        self.assertTrue(
            model1.config.adapters.fusions == model2.config.adapters.fusions
        )
    ...
    def test_adapter_fusion_save_with_head(self):
        ...
        # save & reload model
        with tempfile.TemporaryDirectory() as temp_dir:
            model1.save_adapter_fusion(
                temp_dir, ",".join([name1, name2]), with_head=head_name
            )
            model2.load_adapter_fusion(temp_dir, set_active=True)
