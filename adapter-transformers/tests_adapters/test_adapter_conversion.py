...
class ModelClassConversionTestMixin:
    ...
    def run_test(self, static_model, input_shape=None, label_dict=None):
        ...
        if isinstance(static_model, BertPreTrainedModel):
            ...
            unexpected_keys = [
                k for k in unexpected_keys if "predictions.bias" not in k
            ]
        self.assertEqual(
            0,
            len(unexpected_keys),
        ...
        self.assertTrue(
            torch.allclose(output1[1], output2[1])
        )  # it's not called "logits" for all classes
    ...
    def test_conversion_classification_model(self):
        ...
        model = MODEL_FOR_SEQUENCE_CLASSIFICATION_MAPPING[self.config_class](
            self.config()
        )
        label_dict = {}
        label_dict["labels"] = torch.zeros(
    ...
    def test_equivalent_language_generation(self):
        ...
        static_model = MODEL_FOR_SEQ_TO_SEQ_CAUSAL_LM_MAPPING[self.config_class](
            self.config()
        )
        flex_model = AutoAdapterModel.from_pretrained(
            None, config=self.config(), state_dict=static_model.state_dict()
