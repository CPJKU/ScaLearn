...
def _find_timestamp_sequence(
    ...
    for seq_idx, item in enumerate(sequences):
        ...
        begin_idx = (
            np.where(sequence == timestamp_begin)[0].item()
...
class AutomaticSpeechRecognitionPipeline(ChunkPipeline):
    ...
    def _sanitize_parameters(
        ...
        if return_timestamps is not None:
            ...
            if self.model.config.model_type == "whisper":
                ...
                # used in the decoder.
                if "forced_decoder_ids" not in forward_params.get(
                    "generate_kwargs", {}
                ):
    ...
    def postprocess(
        ...
        if return_timestamps in {"char", "words"} and self.type == "seq2seq_whisper":
            raise ValueError(
                "Whisper cannot return `char` nor `words` timestamps, use `True` instead."
            )
        ...
        if (
            stride
            and self.type in {"seq2seq", "seq2seq_whisper"}
        ...
        if self.type == "ctc_with_lm":
            ...
            if return_timestamps and self.type == "seq2seq_whisper":
                offsets = self.tokenizer.decode(
                    items, skip_special_tokens=skip_special_tokens, output_offsets=True
                )["offsets"]
