...
class TFGenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        input_ids = self._prepare_model_inputs(
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        if model_kwargs.get("token_type_ids") is not None:
            model_kwargs["token_type_ids"] = tf.repeat(
                model_kwargs["token_type_ids"], expand_size, axis=0
            )

        if model_kwargs.get("attention_mask") is not None:
            model_kwargs["attention_mask"] = tf.repeat(
                model_kwargs["attention_mask"], expand_size, axis=0
            )
        ...
        if is_encoder_decoder:
            ...
            encoder_outputs["last_hidden_state"] = tf.repeat(
                encoder_outputs.last_hidden_state, expand_size, axis=0
            )
            model_kwargs["encoder_outputs"] = encoder_outputs
    ...
    def _get_logits_warper(
        ...
        if generation_config.top_k is not None and generation_config.top_k != 0:
            warpers.append(
                TFTopKLogitsWarper(top_k=generation_config.top_k, min_tokens_to_keep=1)
            )
        if generation_config.top_p is not None and generation_config.top_p < 1.0:
            warpers.append(
                TFTopPLogitsWarper(top_p=generation_config.top_p, min_tokens_to_keep=1)
            )
        return warpers
        ...
        **model_kwargs,
        ...
        def beam_search_body_fn(
            ...
            topk_running_sequences = self._gather_beams(
                running_sequences, topk_beam_indices
            )
