        ...
        **kwargs,
        ...
        if saved_model:
            if signatures is None:
                if any(
                    spec.dtype == tf.int32
                    for spec in self.serving.input_signature[0].values()
                ):
                    int64_spec = {
                        key: tf.TensorSpec(
                            shape=spec.shape,
                    ...
                    int64_serving = tf.function(
                        self.eager_serving, input_signature=[int64_spec]
                    )
                    signatures = {
                        "serving_default": self.serving,
        ...
        if index is None:
            ...
            for shard_file, shard in shards.items():
                with h5py.File(
                    ...
                    hdf5_format.save_attributes_to_hdf5_group(
                        shard_file, "layer_names", layers
                    )
