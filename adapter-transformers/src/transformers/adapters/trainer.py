...
class AdapterTrainer(Trainer):
                ...
                if hasattr(self.model.config.adapters, "fusions"):
                    ...
                    if len(config_list) > 0:
                        ...
                        if "difflr_omega" in config_str:
                            ...
                            match_str_c = r"scalearn_layer\..*\.omega"
                            omega_params += [
                                name
                                for name in decay_parameters
    ...
    def _load_from_checkpoint(self, resume_from_checkpoint):
        ...
        if args.deepspeed:
            ...
            if os.path.isdir(resume_from_checkpoint):
                ...
                # self._load_scalearn(resume_from_checkpoint)
    ...
    def _load_adapter_fusions(self, resume_from_checkpoint):
        for file_name in os.listdir(resume_from_checkpoint):
            if os.path.isdir(os.path.join(resume_from_checkpoint, file_name)):
                if "," in file_name and "fusion" in os.listdir(
                    os.path.join(resume_from_checkpoint, file_name)
                ):
    ...
    def _load_scalearn(self, resume_from_checkpoint):
        for file_name in os.listdir(resume_from_checkpoint):
            if os.path.isdir(os.path.join(resume_from_checkpoint, file_name)):
                # check if any file in directory contains string "scalearn"
                if "," in file_name and "scalearn" in os.listdir(
                    os.path.join(resume_from_checkpoint, file_name)
                ):
                    self.model.load_scalearn(
                        os.path.join(resume_from_checkpoint, file_name)
                    )
                    ...
                    if (
                        len(
                            model.base_model.encoder.layer[
                                0
                            ].output.scalearn_layer
                        )
                        > 0
                    ):
                        model.load_scalearn(fusion_dir)
                    else:
...
class AdapterTrainerCallback(TrainerCallback):
    ...
    def on_step_end(
        ...
        **kwargs,
        ...
        model = kwargs.pop("model")
        if self.trainer.train_transfer_layer:
            fusion_reg_loss = model.base_model.get_fusion_regularization_loss()
            scalearn_reg_loss = (
                model.base_model.get_scalearn_regularization_loss()
            )
            if fusion_reg_loss is not None:
                fusion_reg_loss.backward()
            elif scalearn_reg_loss is not None:
                scalearn_reg_loss.backward()
                wandb.log(
                    {"scalearn_reg_loss": scalearn_reg_loss.item()},
