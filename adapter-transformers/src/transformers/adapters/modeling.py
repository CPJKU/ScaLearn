...
class Adapter(nn.Module):
    ...
    def __init__(
        ...
        if config["omega"] is not None:
            self.omega = torch.tensor(config["omega"])
        else:
            self.omega = None
...
class ScaLearn(nn.Module):
    ...
    def forward(self, up_list, residual):
        ...
        # used for ablations
        if self.config["take_mean"]:
            # rescale omegas s.t. sum(omegas) = 1
            omegas = omegas / torch.sum(omegas, dim=0).unsqueeze(0)
            weighted_x = omegas * up_list
        elif self.config["take_softmax"]:
            omegas = torch.softmax(omegas, dim=0)
            weighted_x = omegas * up_list
        else:
            weighted_x = omegas * up_list
...
# ScaLearn++
class ScaLearnPlusPlus(nn.Module):
    def __init__(
        self,
        config,
        dense_size,
        ...
        super(ScaLearnPlusPlus, self).__init__()
        self.config = config
        self.name = name
...
def init_shared_parameters_scalearn(config, in_features, n_adapters: int):
    """
    Create and initialize the parameters shared by all ScaLearn modules
    """
    parameters = nn.ParameterDict()
    if config["per_layer"]:
        if config["learn_omega"]:
            if config["omega_shape"] == "vector":
                omega_shape = (n_adapters, in_features)
            elif config["omega_shape"] == "scalar":
                omega_shape = (n_adapters, 1)
            else:
                raise ValueError("omega_shape must be either vector or scalar")
            if config["omega_init_type"] == "avg":
                omega_init_value = config["omega_init_scale"] / n_adapters
            else:
                raise ValueError("omega_init_type must be avg")
            omega = torch.normal(
                omega_init_value,
...
class NoOpModule(nn.Module):
    """Just do nothing here when probing adapters."""
