...
class AdapterLayerBase(ABC):
    ...
    @abstractmethod
    def add_scalearn_layer(self, adapter_names: Union[List, str]):
        raise NotImplementedError()
...
class AdapterLayer(AdapterLayerBase, nn.Module):
    ...
    def _init_adapter_modules(self):
        self.scalearn_layer = nn.ModuleDict(dict())
    ...
    def add_scalearn_layer(self, adapter_names: Union[List, str], grid_values=None):
        adapter_names = (
            adapter_names
            if isinstance(adapter_names, list)
            else adapter_names.split(",")
        ...
        if self.config.adapters.common_config_value(adapter_names, self.location_key):
            fusion_config = self.config.adapters.get_scalearn(
            ...
            if fusion_config.learn_omega and not fusion_config.per_layer:
                ...
                fusion = NoOpModule()
            fusion.train(self.training)  # make sure training mode is consistent
            self.scalearn_layer[",".join(adapter_names)] = fusion
    ...
    def delete_scalearn_layer(self, adapter_names: Union[List, str]):
        adapter_names = (
            adapter_names if isinstance(adapter_names, str) else ",".join(adapter_names)
        )
        if adapter_names in self.scalearn_layer:
            del self.scalearn_layer[adapter_names]
            ...
            if isinstance(adapter_setup, Fuse):
                if adapter_setup.name in self.adapter_fusion_layer:
                    for param in self.adapter_fusion_layer[
                        ...
                        param.requires_grad = True
                elif adapter_setup.name in self.scalearn_layer:
                    for param in self.scalearn_layer[adapter_setup.name].parameters():
                        param.requires_grad = True
                ...
                if isinstance(sub_setup, Fuse):
                    if sub_setup.name in self.adapter_fusion_layer:
                        for param in self.adapter_fusion_layer[
                            ...
                            param.requires_grad = True
                    elif sub_setup.name in sub_setup.name in self.scalearn_layer:
                        for param in self.scalearn_layer[sub_setup.name].parameters():
                            param.requires_grad = True
    ...
    def adapter_stack(
        ...
        for i, adapter_stack_layer in enumerate(adapter_setup):
            ...
            if isinstance(adapter_stack_layer, Fuse):
                ...
                elif len(self.scalearn_layer) == 1:
                    hidden_states = self.scalearn(
                        adapter_stack_layer,
                        hidden_states,
                        input_tensor,
                        layer_norm,
    ...
    def scalearn(
        ...
        # config of _last_ fused adapter is significant
        fusion_config = self.config.adapters.get_scalearn(adapter_setup.name)
        last_adapter = self.adapters[adapter_setup.last()]
        hidden_states, query, residual = last_adapter.pre_forward(
            hidden_states, input_tensor, layer_norm, fusion_config=fusion_config
        )
        ...
        if len(up_dict) > 0:
            ...
            if not fusion_config.learn_omega:
                ...
                # add back residual; not included before!
                hidden_states += residual
            else:
                up_list = torch.stack(list(up_dict.values()))
                up_list = up_list.permute(1, 2, 0, 3)

                hidden_states = self.scalearn_layer[adapter_setup.name](
                    up_list, residual
                )
    ...
    def adapter_layer_forward(
        self, hidden_states, residual_input, layer_norm, task=None
    ):
