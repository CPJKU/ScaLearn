...
@add_start_docstrings(
    "MBART Model with the option to add multiple flexible prediction heads on top.",
    MBART_START_DOCSTRING,
)
class MBartAdapterModel(
    ...
    def forward(
        ...
        **kwargs
        ...
        if (
            "labels" in kwargs
            or "start_positions" in kwargs
            and "end_positions" in kwargs
        ):
            use_cache = False
        ...
        if input_ids is not None and x.shape[1] == input_ids.shape[1]:
            ...
            cls_representation = x[eos_mask, :].view(x.size(0), -1, x.size(-1))[
                :, -1, :
            ]
        else:
