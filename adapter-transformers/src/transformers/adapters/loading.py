...
from .utils import (
    ...
    HEAD_CONFIG_NAME,
    HEAD_WEIGHTS_NAME,
    SCALEARN_CONFIG_NAME,
    SCALEARN_WEIGHTS_NAME,
    WEIGHTS_NAME,
    AdapterType,
...
class ScaLearnLoader(WeightsLoader):
    """
    A class providing methods for saving and loading AdapterFusion modules from the file system.
    ...
    def __init__(self, model, error_on_missing=True):
        super().__init__(model, SCALEARN_WEIGHTS_NAME, SCALEARN_CONFIG_NAME)
        self.error_on_missing = error_on_missing

    def filter_func(self, adapter_fusion_name):
        return lambda x: "scalearn_layer.".format() in x
    ...
    def save(self, save_directory: str, name: str, meta_dict=None):
        """
        Saves a ScaLearn module into the given directory.
        ...
        if name not in self.model.config.adapters.fusions:
            if self.error_on_missing:
                raise ValueError(f"Unknown ScaLearn '{name}'.")
            else:
                logger.debug(f"No ScaLearn with name '{name}' available.")
                return
        ...
        adapter_fusion_config = self.model.config.adapters.get_scalearn(name)
    ...
    def load(self, save_directory, load_as=None, loading_info=None, **kwargs):
        """
        Loads a ScaLearn module from the given directory.
        ...
        Returns:
            ...
            and the name of the loaded weights.
        """
        if not exists(join(save_directory, SCALEARN_WEIGHTS_NAME)):
            if self.error_on_missing:
                raise ValueError(
                    "Loading path should be a directory where AdapterFusion is saved."
                )
