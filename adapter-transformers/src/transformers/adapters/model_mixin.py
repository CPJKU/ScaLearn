...
from .modeling import (
    ...
    init_shared_parameters_scalearn,
)
...
class ModelAdaptersMixin(PushAdapterToHubMixin, ABC):
    ...
    def train_fusion(
        ...
        self.train_transfer_layer(adapter_setup, unfreeze_adapters=unfreeze_adapters)

    def train_transfer_layer(
        self,
        adapter_setup: Union[list, AdapterCompositionBlock],
        unfreeze_adapters=False,
    ):
        """Sets the model into mode for training of a transfer layer (ScaLearn/Fusion) determined by a list of adapter names."""
        self.train()
        self.freeze_model(True)
    ...
    def add_shared_adapter(
        ...
        """
        Adds a new shared adapter module of the specified type to the model, i.e., ProPETL-adapter.
                ...
                if self.config.model_type in SUBMODEL_NAMES:
                    ...
                    if all(hidden_sizes[0] == h for h in hidden_sizes):
                        self.base_model.shared_parameters[
                            ...
                            adapter_config, hidden_sizes[0], device=self.device
                        )
                    else:
                        raise ValueError(
                    ...
                    self.base_model.shared_parameters[
                        ...
                        adapter_config, self.config.hidden_size, device=self.device
                    )
        # Prefix Tuning
        for module in self.modules():
            if isinstance(module, PrefixTuningPool):
    ...
    def add_scalearn(
        ...
        """
        Adds ScaLearn to the model with alll the necessary configurations and weight initializations

        Args:
            adapter_names (Fuse or list or str): ScaLearn layer to add. Can be either:
            ...
            config (str or dict): adapter fusion configuration, can be either:
                ...
                - the path to a file containing the adapter fusion configuration
            overwrite_ok (bool, optional):
                Overwrite a ScaLearn layer with the same name if it exists. By default (False), an exception is
                thrown.
            set_active (bool, optional):
                Activate the added ScaLearn. By default (False), ScaLearn is added but not activated.
        """
        if isinstance(adapter_names, Fuse):
            adapter_names = adapter_names.children
        elif isinstance(adapter_names, str):
        ...
        if isinstance(config, dict):
            config = ScaLearnConfig.from_dict(
                config
            )  # ensure config is ok and up-to-date
        # In case adapter already exists and we allow overwriting, explicitly delete the existing one first
        if overwrite_ok and self.config.adapters.get_fusion(adapter_names) is not None:
            self.delete_scalearn(adapter_names)
        self.config.adapters.add_scalearn(
            adapter_names, config=config, grid_values=grid_values
        )
        self.apply_to_adapter_layers(
            lambda i, layer: layer.add_scalearn_layer(
        ...
        scalearn_config = self.config.adapters.get_scalearn(
            adapter_names, grid_values=grid_values
        )
        if scalearn_config.per_layer:
            self.base_model.shared_parameters[
                ",".join(adapter_names)
            ] = init_shared_parameters_scalearn(
                scalearn_config,
    ...
    def delete_scalearn(self, adapter_names: Union[Fuse, list, str]):
    ...
    def save_scalearn(
        ...
        """
        Saves a ScaLearn layer and its configuration file to a directory so that it can be shared or reloaded
        using `load_scalearn()`.

        Args:
            save_directory (str): Path to a directory where the ScaLearn should be saved.
            adapter_names (Union[Fuse, list, str]): ScaLearn to be saved.

        Raises:
            ValueError: If the given ScaLearn name is invalid.
        """
        if isinstance(adapter_names, Fuse):
            adapter_fusion_name = ",".join(adapter_names.children)
        elif isinstance(adapter_names, list):
    ...
    def load_scalearn(
        ...
        **kwargs,
        ...
        """
        Loads a pre-trained ScaLearn layer from the local file system.

        Args:
            adapter_fusion_name_or_path (str):
                a path to a directory containing ScaLearn weights saved using `model.save_scalearn()`.
            load_as (str, optional): Load the ScaLearn using this name.
                    By default, the name with which the ScaLearn layer was saved will be used.
            set_active (bool, optional):
                Activate the loaded ScaLearn. By default (False), the ScaLearn is loaded but not activated.

        Returns:
            str: The name with which the ScaLearn was added to the model.
        """
    ...
    def save_all_adapter_fusions(
        ...
        for name in self.config.adapters.fusions:
            if len(self.base_model.encoder.layer[0].output.scalearn_layer) > 0:
                adapter_fusion_config = self.config.adapters.get_scalearn(name)
            else:
    ...
    def save_all_scalearns(
        ...
        """
        Saves all ScaLearn layers of this model together with their configuration to subfolders of the given
        location.

        Args:
            save_directory (str): Path to a directory where the ScaLearn layers should be saved.
        """
        os.makedirs(save_directory, exist_ok=True)
        for name in self.config.adapters.fusions:
            adapter_fusion_config = self.config.adapters.get_scalearn(name)
    ...
    def get_scalearn_regularization_loss(self):
        reg_loss = None
        ...
        if len(self.shared_parameters) > 0:
            for name, param in self.shared_parameters.named_parameters():
                if "omega" in name:
                    module_name = list(
                        self.encoder.layer[0].output.scalearn_layer
                    )[0]
                    config = (
                        self.encoder.layer[0]
                        .output.scalearn_layer[module_name]
                        .config
                    )
            ...
            for i, layer in self.iter_layers():
                for module in layer.modules():
                    if isinstance(module, AdapterLayer):
                        for _, layer_fusion in module.scalearn_layer.items():
                            if hasattr(
                                list(module.scalearn_layer.items())[0][1], "omega"
                            ):
                                module_name = list(module.scalearn_layer)[0]
                                config = module.scalearn_layer[module_name].config
                                if config["regularization"]:
...
class ModelWithHeadsAdaptersMixin(ModelAdaptersMixin):
    ...
    def train_transfer_layer(
        ...
        """
        Sets the model into mode for training of a transfer layer determined by a list of adapter names. If
        self.base_model is self, must inherit from a class that implements this method, to preclude infinite recursion
        """
        if self.base_model is self:
            super().train_transfer_layer(
                adapter_setup, unfreeze_adapters=unfreeze_adapters
            )
        else:
            self.base_model.train_transfer_layer(
                adapter_setup, unfreeze_adapters=unfreeze_adapters
            )
    ...
    def train_scalearn(
        ...
        """
        Sets the model into mode for training of ScaLearn determined by a list of adapter names. If
        self.base_model is self, must inherit from a class that implements this method, to preclude infinite recursion
        """
        if self.base_model is self:
            super().train_scalearn(
                adapter_setup, unfreeze_adapters=unfreeze_adapters
            )
        else:
            self.base_model.train_scalearn(
                adapter_setup, unfreeze_adapters=unfreeze_adapters
            )
    ...
    def save_scalearn(
        ...
        Args:
            ...
            adapter_names (Union[Fuse, list, str]): ScaLearn to be saved.
            with_head (Union[bool, str]):
                If True, will save a head with the same name as the ScaLearnLayer. If a string, this will be used
                as the name of the head to be saved.
