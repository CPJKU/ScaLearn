...
class BiaffineParsingHead(PredictionHead):
    ...
    def build(self, model):
        self.biaffine_arcs = Biaffine(
            n_in=model.config.hidden_size, bias_x=True, bias_y=False
        )
        self.biaffine_rels = Biaffine(
            n_in=model.config.hidden_size,
    ...
    def forward(
        ...
        **kwargs,
        ...
        word_outputs_heads = torch.cat(
            [cls_output.unsqueeze(1), word_outputs_deps], dim=1
        )
        ...
        loss = self._get_loss(
            arc_preds, rel_preds, labels_arcs, labels_rels, self.loss_fn
        )
