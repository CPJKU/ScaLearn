...
class PredictionHead(nn.Sequential):
    ...
    def build(self, model):
        ...
        if "dropout_prob" in self.config:
            ...
            hasattr(model_config, "classifier_dropout")
            and model_config.classifier_dropout is not None
        ):
        ...
        for l_id in range(self.config["layers"]):
            ...
            if l_id < self.config["layers"] - 1:
                ...
                if self.config["activation_function"]:
                    pred_head.append(
                        Activation_Function_Class(self.config["activation_function"])
                    )
            else:
...
class ModelWithFlexibleHeadsAdaptersMixin(ModelWithHeadsAdaptersMixin):
    ...
    @active_head.setter
    def active_head(
        self, head_name_or_list: Union[str, List[str], AdapterCompositionBlock]
    ):
        if isinstance(head_name_or_list, str):
            if head_name_or_list and head_name_or_list not in self.heads:
                raise ValueError(
                    f"Model does not contain a head with name '{head_name_or_list}'."
                )
            ...
            if head_name_or_list:
                self.config.label2id = self.heads[head_name_or_list].config.get(
                    "label2id", None
                )
    ...
    def set_active_adapters(
        ...
        if self.active_adapters:
            ...
            if head_setup and all(head in self.heads for head in head_setup):
                ...
                logger.info(
                    "Could not identify valid prediction head(s) from setup '{}'.".format(
        ...
        if head.name not in self.heads or overwrite_ok:
            ...
            if "label2id" in head.config.keys() and head.config["label2id"] is None:
                if "num_labels" in head.config.keys():
                    head.config["label2id"] = {
                        "LABEL_" + str(num): num
                        for num in range(head.config["num_labels"])
                    }
                if "num_choices" in head.config.keys():
                    head.config["label2id"] = {
                        "LABEL_" + str(num): num
                        for num in range(head.config["num_choices"])
                    }
    ...
    def delete_head(self, head_name: str):
        ...
        if head_name not in self.config.prediction_heads:
            logger.info(
                "No prediction head '%s' found for deletion. Skipping.", head_name
            )
            return
        ...
        **kwargs,
        ...
        if isinstance(self.active_head, BatchSplit):
            ...
            for i, head in enumerate(self.active_head):
                ...
                batch_idx = range(
                    sum(self.active_head.batch_sizes[:i]),
                    sum(self.active_head.batch_sizes[: i + 1]),
                )
                kwargs["labels"] = labels[batch_idx] if labels is not None else None
                head_inputs, head_cls_input = _get_head_input(
                    all_outputs, cls_output, batch_idx
                )
                # head_attention = attention_mask[batch_idx] if attention_mask is not None else None
                head_output = head_module(
                    head_inputs, head_cls_input, attention_mask, return_dict, **kwargs
                )
                head_outputs.append(head_output)
            combined_loss = (
                sum([out["loss"] for out in head_outputs])
                if all(
                    "loss" in out and out["loss"] is not None for out in head_outputs
                )
                else None
            )
            return_output = MultiHeadOutput(
                head_outputs=head_outputs, loss=combined_loss
            )
        elif self.has_parallel_adapters or isinstance(self.active_head, Parallel):
            if (
                len(self.active_head)
                != self.config.adapters.active_setup.parallel_channels
            ):
                raise ValueError(
                    "The number of parallel adapters and the number of active heads must match."
                )
            orig_batch_size = (
                all_outputs[0].shape[0]
                // self.config.adapters.active_setup.parallel_channels
            )
            head_outputs = []
            for i, head in enumerate(self.active_head):
    ...
    def get_labels_dict(self, head_name=None):
        ...
        if (
            "label2id" in self.heads[head_name].config.keys()
            and self.heads[head_name].config["label2id"] is not None
        ):
            return {
                id_: label
                for label, id_ in self.heads[head_name].config["label2id"].items()
            }
        else:
            return None
