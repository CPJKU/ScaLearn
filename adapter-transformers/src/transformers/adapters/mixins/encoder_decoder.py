...
class EncoderDecoderModelAdaptersMixin(
    EmbeddingAdaptersMixin, InvertibleAdaptersMixin, ModelAdaptersMixin
):
    ...
    def _init_adapter_modules(self):
        if not isinstance(self.encoder, ModelAdaptersMixin) or not isinstance(
            self.decoder, ModelAdaptersMixin
        ):
            return
        ...
        self.enable_invertible_adapters = (
            self.encoder.base_model.enable_invertible_adapters
        )
        self.invertible_adapters_forward = (
            self.encoder.base_model.invertible_adapters_forward
        )
        # Decoder should use invertible adapters of encoder
        self.decoder.base_model.invertible_adapters = (
            self.encoder.base_model.invertible_adapters
        )
        self.decoder.base_model.add_invertible_adapter = lambda *args: None
        self.decoder.base_model.get_invertible_adapter = (
            self.encoder.base_model.get_invertible_adapter
        )
        ...
        def decoder_invertible_adapters_forward(hidden_states, rev=False):
            if rev:
                return self.encoder.base_model.invertible_adapters_forward(
                    hidden_states, rev=True
                )
            else:
                return hidden_states

        self.decoder.base_model.invertible_adapters_forward = (
            decoder_invertible_adapters_forward
        )
