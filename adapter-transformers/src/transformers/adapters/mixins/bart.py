    ...
    def iter_layers(self) -> Iterable[Tuple[int, nn.Module]]:
        if hasattr(self, "encoder"):
            ...
            for i, layer in enumerate(
                self.decoder.layers, start=len(self.encoder.layers)
            ):
                yield i, layer
        else:
...
class BartModelWithHeadsAdaptersMixin(
    EmbeddingAdaptersWrapperMixin, ModelWithHeadsAdaptersMixin
):
    pass
