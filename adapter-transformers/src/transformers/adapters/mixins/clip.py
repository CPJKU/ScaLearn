...
class CLIPTextModelAdaptersMixin(
    EmbeddingAdaptersMixin, InvertibleAdaptersWrapperMixin, ModelAdaptersMixin
):
...
class CLIPModelAdaptersMixin(
    EmbeddingAdaptersWrapperMixin, InvertibleAdaptersWrapperMixin, ModelAdaptersMixin
):
    ...
    def iter_layers(self) -> Iterable[Tuple[int, nn.Module]]:
        ...
        for i, layer in enumerate(
            self.vision_model.encoder.layers, start=len(self.text_model.encoder.layers)
        ):
            yield i, layer
