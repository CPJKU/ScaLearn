...
class AlbertLayer(AlbertEncoderLayerAdaptersMixin, nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.output_adapters(
            ffn_output, attention_output[0], self.full_layer_layer_norm
        )
...
class AlbertForPreTraining(AlbertModelWithHeadsAdaptersMixin, AlbertPreTrainedModel):
    ...
    def forward(
        ...
        prediction_scores = self.predictions(
            sequence_output, inv_lang_adapter=self.albert.get_invertible_adapter()
        )
...
class AlbertMLMHead(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.Tensor, inv_lang_adapter=None
    ) -> torch.Tensor:
...
class AlbertForMaskedLM(AlbertModelWithHeadsAdaptersMixin, AlbertPreTrainedModel):
    ...
    def forward(
        ...
        prediction_scores = self.predictions(
            sequence_outputs, inv_lang_adapter=self.albert.get_invertible_adapter()
        )
...
class AlbertForSequenceClassification(
    AlbertModelWithHeadsAdaptersMixin, AlbertPreTrainedModel
):
    def __init__(self, config: AlbertConfig):
        super().__init__(config)
