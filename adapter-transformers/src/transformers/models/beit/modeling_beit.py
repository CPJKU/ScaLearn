...
class BeitSelfAttention(nn.Module):
    ...
    def forward(
        ...
        key_layer, value_layer, _ = self.prefix_tuning(
            key_layer, value_layer, hidden_states
        )
...
class BeitAttention(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.attention = BeitSelfAttention(
            config, window_size=window_size, location_key=location_key
        )
...
class BeitLayer(BeitLayerAdaptersMixin, nn.Module):
    ...
    def __init__(
        ...
        self.seq_len_dim = 1
        self.attention = BeitAttention(
            config, window_size=window_size, location_key="self"
        )
    ...
    def forward(
        ...
        layer_output = self.output_adapters.adapter_layer_forward(
            self.drop_path(layer_output), hidden_states, None
        )
