...
class T5DenseActDense(nn.Module):
    def __init__(self, config: T5Config):
        super().__init__()
        self.wi = LoRALinear(
            config.d_model, config.d_ff, "intermediate", config, bias=False
        )
...
class T5Attention(nn.Module):
    def __init__(
        ...
        self.q = LoRALinear(
            self.d_model, self.inner_dim, "selfattn", config, attn_key="q", bias=False
        )
        self.k = LoRALinear(
            self.d_model, self.inner_dim, "selfattn", config, attn_key="k", bias=False
        )
        self.v = LoRALinear(
            self.d_model, self.inner_dim, "selfattn", config, attn_key="v", bias=False
        )
    ...
    def forward(
        ...
        key_states, value_states, mask = self.prefix_tuning(
            key_states, value_states, hidden_states, mask
        )
...
class T5ForConditionalGeneration(
    T5ModelWithHeadsAdaptersMixin, T5ModelAdaptersMixin, T5PreTrainedModel
):
    _keys_to_ignore_on_load_missing = [
        r"encoder.embed_tokens.weight",
        r"decoder.embed_tokens.weight",
