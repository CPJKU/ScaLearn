...
class BartAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = LoRALinear(
            embed_dim, embed_dim, "selfattn", config, attn_key="k", bias=bias
        )
        self.v_proj = LoRALinear(
            embed_dim, embed_dim, "selfattn", config, attn_key="v", bias=bias
        )
        self.q_proj = LoRALinear(
            embed_dim, embed_dim, "selfattn", config, attn_key="q", bias=bias
        )
...
class BartEncoderLayer(BartEncoderLayerAdaptersMixin, nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.fc1 = LoRALinear(
            self.embed_dim, config.encoder_ffn_dim, "intermediate", config
        )
    ...
    def forward(
        ...
        hidden_states = self.attention_adapters(
            hidden_states, residual, self.self_attn_layer_norm
        )
        ...
        hidden_states = self.output_adapters(
            hidden_states, residual, self.final_layer_norm
        )
...
class BartDecoderLayer(BartDecoderLayerAdaptersMixin, nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states = self.cross_attention_adapters(
                hidden_states, residual, self.encoder_attn_layer_norm
            )
...
class BartForConditionalGeneration(
    BartModelWithHeadsAdaptersMixin, BartPretrainedModel
):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [
        r"final_logits_bias",
