...
def fix_jukebox_keys(state_dict, model_state_dict, key_prefix, mapping):
    ...
    re_encoder_block_conv_in = re.compile(
        "encoders.(\d*).level_blocks.(\d*).model.(\d*).(\d).(bias|weight)"
    )
    ...
    re_encoder_block_proj_out = re.compile(
        "encoders.(\d*).level_blocks.(\d*).model.(\d*).(bias|weight)"
    )

    re_decoder_block_conv_out = re.compile(
        "decoders.(\d*).level_blocks.(\d*).model.(\d*).(\d).(bias|weight)"
    )
    ...
    re_decoder_block_proj_in = re.compile(
        "decoders.(\d*).level_blocks.(\d*).model.(\d*).(bias|weight)"
    )

    re_prior_cond_conv_out = re.compile(
        "conditioner_blocks.(\d*).cond.model.(\d*).(\d).(bias|weight)"
    )
    ...
    re_prior_cond_proj_in = re.compile(
        "conditioner_blocks.(\d*).cond.model.(\d*).(bias|weight)"
    )
