...
class GPTJAttention(nn.Module):
    def __init__(self, config):
        ...
        self.k_proj = LoRALinear(
            self.embed_dim, self.embed_dim, "selfattn", config, attn_key="k", bias=False
        )
        self.v_proj = LoRALinear(
            self.embed_dim, self.embed_dim, "selfattn", config, attn_key="v", bias=False
        )
        self.q_proj = LoRALinear(
            self.embed_dim, self.embed_dim, "selfattn", config, attn_key="q", bias=False
        )
    ...
    def forward(
        ...
        key, value, attention_mask = self.prefix_tuning(
            key, value, hidden_states, attention_mask
        )
...
class GPTJBlock(GPTJDecoderBlockAdaptersMixin, nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.output_adapters(
            feed_forward_hidden_states, hidden_states, None
        )
...
class GPTJForSequenceClassification(
    GPTJModelWithHeadsAdaptersMixin, GPTJPreTrainedModel
):
    _keys_to_ignore_on_load_missing = [
        r"h\.\d+\.attn\.masked_bias",
        r"h\.\d+\.attn\.bias",
