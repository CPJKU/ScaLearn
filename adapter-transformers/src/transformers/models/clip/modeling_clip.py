...
class CLIPAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.k_proj = LoRALinear(
            self.embed_dim, self.embed_dim, "selfattn", config, attn_key="k"
        )
        self.v_proj = LoRALinear(
            self.embed_dim, self.embed_dim, "selfattn", config, attn_key="v"
        )
        self.q_proj = LoRALinear(
            self.embed_dim, self.embed_dim, "selfattn", config, attn_key="q"
        )
        self.out_proj = nn.Linear(self.embed_dim, self.embed_dim)

        self.prefix_tuning = PrefixTuningShim(
            "self_prefix", config, add_model_type_to_key=True
        )
    ...
    def forward(
        ...
        if causal_attention_mask is not None:
            prefix_mask = torch.ones(
                bsz,
                1,
                causal_attention_mask.size(2),
                src_len - causal_attention_mask.size(-1),
            ).to(causal_attention_mask.device)
            causal_attention_mask = torch.cat(
                [prefix_mask, causal_attention_mask], dim=-1
            )
...
class CLIPEncoderLayer(CLIPEncoderLayerAdaptersMixin, nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.attention_adapters(
            hidden_states, residual, layer_norm=None
        )
