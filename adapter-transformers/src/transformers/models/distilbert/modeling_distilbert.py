...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, config: PretrainedConfig):
        ...
        self.q_lin = LoRALinear(
            config.dim, config.dim, "selfattn", config, attn_key="q"
        )
        self.k_lin = LoRALinear(
            config.dim, config.dim, "selfattn", config, attn_key="k"
        )
        self.v_lin = LoRALinear(
            config.dim, config.dim, "selfattn", config, attn_key="v"
        )
...
class TransformerBlock(DistilBertTransfomerBlockAdaptersMixin, nn.Module):
    ...
    def forward(
        ...
        sa_output = self.attention_adapters(
            sa_output, x, self.sa_layer_norm
        )  # (bs, seq_length, dim)
...
class DistilBertForMaskedLM(
    DistilBertModelWithHeadsAdaptersMixin, DistilBertPreTrainedModel
):
