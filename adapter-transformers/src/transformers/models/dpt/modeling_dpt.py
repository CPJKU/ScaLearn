...
class DPTViTHybridEmbeddings(nn.Module):
    ...
    def __init__(self, config, feature_size=None):
        ...
        self.residual_feature_map_index = [
            0,
            1,
        ]  # Always take the output of the first and second backbone stage
    ...
    def forward(
        ...
        output_hidden_states = [
            backbone_output.feature_maps[index]
            for index in self.residual_feature_map_index
        ]
...
class DPTReassembleStage(nn.Module):
    ...
    def _init_reassemble_dpt_hybrid(self, config):
        ...
        if config.readout_type != "project":
            raise ValueError(
                f"Readout type {config.readout_type} is not supported for DPT-Hybrid."
            )
        ...
        for i, hidden_state in enumerate(hidden_states):
            if i not in self.neck_ignore_stages:
                ...
                hidden_state = hidden_state.reshape(
                    batch_size, size, size, num_channels
                )
...
class DPTModel(DPTPreTrainedModel):
    ...
    def forward(
        ...
        embedding_last_hidden_states = (
            embedding_output[0]
            if not return_dict
            else embedding_output.last_hidden_states
        )
    ...
    def forward(
        ...
        if not self.config.is_hybrid:
            ...
            backbone_hidden_states = (
                outputs.intermediate_activations if return_dict else list(outputs[-1])
            )
