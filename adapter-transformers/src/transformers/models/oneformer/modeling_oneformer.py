    ...
    def __init__(
        ...
        if self.contrastive_temperature is not None:
            self.logit_scale = nn.Parameter(
                torch.ones([]) * np.log(1 / contrastive_temperature)
            )
...
class OneFormerPixelDecoderEncoderMultiscaleDeformableAttention(nn.Module):
    ...
    def forward(
        ...
        # CPU
        output = multiscale_deform_attn_core_pytorch(
        ...
        self.decoder = nn.ModuleList(
            [
                OneFormerTextTransformerDecoderLayer(
                    transformer_width, transformer_heads, dropout
                )
...
class OneFormerTextTransformer(nn.Module):
    def __init__(
        ...
        self.layers = nn.Sequential(
            *[
                OneFormerTextTransformerLayer(width, heads, attn_mask)
