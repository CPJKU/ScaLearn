        ...
        if state.is_world_process_zero:
            ...
            if "train/loss" in logs.keys():
                ...
                if not hasattr(model.base_model, "encoder"):
                    return
                elif not hasattr(model.base_model.encoder, "layer"):
                    return
                elif not hasattr(model.base_model.encoder.layer[0], "output"):
                    return
                elif not hasattr(
                    model.base_model.encoder.layer[0].output, "scalearn_layer"
                ):
                    return
                named_params = list(
                    model.base_model.encoder.layer[
                        0
                    ].output.scalearn_layer.named_parameters()
                )
                if len(named_params) > 0:
                    if hasattr(
                        ...
                        for i, layer in enumerate(range(len(model.base_model.encoder.layer))):
                            omegas = (
                                torch.mean(
                                    list(
                                        model.base_model.encoder.layer[
                        ...
                        # if some dimension greater than 16 (n_tasks): skip
                        if all_omegas.shape[1] > 16:
                            return
                        # if more than 12 layers, make font smaller
                        if all_omegas.shape[0] > 12:
                        ...
                        np.save(
                            args.output_dir
                            + f"/omegas/omegas_{state.global_step}.npy",
                            all_omegas,
                        )
                        plt.close()
                        self._wandb.log(
                    ...
                    #  if it has attr phm_rule --> not ScaLearn (but Compacter)
                    if "phm_rule" in first_item[0]:
                        return
                    logger.info("Logging Compositionality")
                    for (
                        name,
                        param,
                    ) in model.base_model.shared_parameters.named_parameters():
                        params = (
                            torch.mean(
                                param,
                                dim=1,
                        ...
                        ax.set_yticklabels(name.split(".")[0].split(","), rotation=0)
                        ...
                        np.save(
                            args.output_dir
                            + f"/params/params-{current_name}_{state.global_step}.npy",
                            params,
                        )
                        plt.close()
                        self._wandb.log(
