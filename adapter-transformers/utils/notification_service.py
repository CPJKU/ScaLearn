...
class Message:
    ...
    def error_out(
        ...
        if ci_title:
            ci_title_block = {
                "type": "section",
                "text": {"type": "mrkdwn", "text": ci_title},
            }
...
if __name__ == "__main__":
    ...
    runner_not_available = (
        True if runner_status is not None and runner_status != "success" else False
    )
    runner_failed = (
        True
        if runner_env_status is not None and runner_env_status != "success"
        else False
    )
    setup_failed = (
    ...
    if runner_not_available or runner_failed or setup_failed:
        Message.error_out(
            title, ci_title, runner_not_available, runner_failed, setup_failed
        )
        exit(0)
