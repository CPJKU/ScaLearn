...
class UDTrainingArguments(TrainingArguments):
    ...
    **which relate to the training loop itself**.
    ...
    decode_mode: str = field(
        default="greedy",
        metadata={"help": "Whether to use mst decoding or greedy decoding"},
    )
...
class ParsingMetric(Metric):
    ...
    def add(
        ...
        unwrapped = self.unpack(
            predicted_indices, predicted_labels, gold_indices, gold_labels
        )
...
class DependencyParsingTrainer(Trainer):
    ...
    def _prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description):
            ...
            predictions_rels = predictions_rels[
                torch.arange(len(labels_arcs)), labels_arcs
            ]
        ...
        return PredictionOutput(
            predictions=predictions_rels, label_ids=None, metrics=results
        )
