...
class ModelArguments:
    ...
    replace_embeddings: bool = field(
        default=False, metadata={"help": "Whether or not to replace embeddings."}
    )
    leave_out_twelvth: bool = field(
        default=False,
        metadata={"help": "Whether or not to leave out adapters in twelvth layer"},
    )
    do_lower_case: bool = field(
        default=False,
        metadata={"help": "Set this flag when using uncased model/tokenizer"},
    )
    is_japanese: bool = field(
        default=False,
        metadata={"help": "Set this to true when using Japanese model/tokenizer"},
    )
    mecab_dir: Optional[str] = field(
        default=None,
        metadata={
            "help": "Path to mecab installation. Required when using Japanese model/tokenizer"
        },
    )
    mecab_dic_dir: Optional[str] = field(
        default=None,
        metadata={
            "help": "Path to mecab dictionary. Required when using Japanese model/tokenizer"
        },
    )
...
class DataTrainingArguments:
    ...
    task_name: str = field(
        metadata={
            "help": "The identifier of the Universal Dependencies dataset to train on."
        }
    )
...
def main():
    ...
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, UDTrainingArguments, AdapterArguments)
    )
    ...
    if data_args.use_mock_data:
        ...
        dataset_builder = load_dataset_builder(
            "universal_dependencies", data_args.task_name
        )
        mock_dl_manager = MockDownloadManager(
            "universal_dependencies", dataset_builder.config, Version("2.7.0")
        )
        dataset_builder.download_and_prepare(
            dl_manager=mock_dl_manager, ignore_verifications=True
        )
    ...
    trainer_class = (
        DependencyParsingAdapterTrainer
        if adapter_args.train_adapter
        else DependencyParsingTrainer
    )
            ...
            if adapter_args.train_adapter:
                ...
                if adapter_args.load_lang_adapter:
                    ...
                    lang_adapter_name = model.load_adapter(
                        os.path.join(
                            training_args.output_dir, "best_model", lang_adapter_name
                        )
                        if training_args.do_train
