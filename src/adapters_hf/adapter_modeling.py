...
from .adapter_outputs import (
    AdapterOutput,
    AdapterT5BlockOutput,
    LayerNormOutput,
    SamplerOutput,
)
from .adapter_utils import Activations, LayerNormHyperNet, TaskHyperNet, linear_layer
...
class Adapter(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.down_sampler = linear_layer(
            self.input_dim, self.down_sample_size, std=self.weight_init_range
        )
...
class AdapterLayersHyperNetController(nn.Module):
    ...
    def __init__(self, config, num_layers=6):
        ...
        self.adp_after_self = config.adp_after_self
        ...
        self.feed_forward_up_sampler_hyper_nets = nn.ModuleList(
            [
                AdapterLayersHyperNet(config, self.input_dim, self.down_sample_size)
                for _ in range(self.num_layers)
            ]
        )
        self.feed_forward_down_sampler_hyper_nets = nn.ModuleList(
            [
                AdapterLayersHyperNet(config, self.down_sample_size, self.input_dim)
                for _ in range(self.num_layers)
            ]
        )
        if self.adp_after_self:
            self.self_attention_up_sampler_hyper_nets = nn.ModuleList(
                [
                    AdapterLayersHyperNet(config, self.input_dim, self.down_sample_size)
                    for _ in range(self.num_layers)
                ]
            )
            self.self_attention_down_sampler_hyper_nets = nn.ModuleList(
                [
                    AdapterLayersHyperNet(config, self.down_sample_size, self.input_dim)
                    for _ in range(self.num_layers)
                ]
        ...
        if self.add_layer_norm_before_adapter:
            self.feed_forward_pre_layernorm_hypernets = nn.ModuleList(
                [LayerNormHyperNet(config) for _ in range(self.num_layers)]
            )
            if self.adp_after_self:
                self.self_attention_pre_layernorm_hypernets = nn.ModuleList(
                    [LayerNormHyperNet(config) for _ in range(self.num_layers)]
                )
        if self.add_layer_norm_after_adapter:
            self.feed_forward_post_layernorm_hypernets = nn.ModuleList(
                [LayerNormHyperNet(config) for _ in range(self.num_layers)]
            )
            if self.adp_after_self:
                self.self_attention_post_layernorm_hypernets = nn.ModuleList(
                    [LayerNormHyperNet(config) for _ in range(self.num_layers)]
                )
    ...
    def get_embedding(self, task_embedding, layer_id):
        ...
        embeddings = self.task_hypernet(task_embedding.view(-1))
        if self.unique_hyper_net_layer_norm:
    ...
    def forward(self, task_embedding, layer_id):
        ...
        feed_forward_down = self.feed_forward_down_sampler_hyper_nets[layer_id](
            embeddings
        )
        feed_forward_up = self.feed_forward_up_sampler_hyper_nets[layer_id](embeddings)
        if self.adp_after_self:
            self_attention_down = self.self_attention_down_sampler_hyper_nets[layer_id](
                embeddings
            )
            self_attention_up = self.self_attention_up_sampler_hyper_nets[layer_id](
                embeddings
            )
        ...
        if self.add_layer_norm_before_adapter:
            weight, bias = self.feed_forward_pre_layernorm_hypernets[layer_id](
                embeddings
            )
            feed_forward_output.pre_norm = LayerNormOutput(weight=weight, bias=bias)
            if self.adp_after_self:
                weight, bias = self.self_attention_pre_layernorm_hypernets[layer_id](
                    embeddings
                )
        ...
        if self.add_layer_norm_after_adapter:
            weight, bias = self.feed_forward_post_layernorm_hypernets[layer_id](
                embeddings
            )
            feed_forward_output.post_norm = LayerNormOutput(weight=weight, bias=bias)
            if self.adp_after_self:
                weight, bias = self.self_attention_post_layernorm_hypernets[layer_id](
                    embeddings
                )
                self_attention_output.post_norm = LayerNormOutput(
                    weight=weight, bias=bias
                )
        return AdapterT5BlockOutput(
...
class AdapterLayersOneHyperNetController(nn.Module):
    ...
    def __init__(self, config, num_layers=6):
        ...
        config.task_embedding_dim = (
            ...
            if not self.adp_after_self
            else self.task_embedding_dim * 3
        )
        self.task_hypernet = TaskHyperNet(config)
        config.task_embedding_dim = self.task_embedding_dim
        self.unique_hyper_net_layer_norm = config.unique_hyper_net_layer_norm
