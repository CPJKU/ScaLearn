...
class TaskEmbeddingController(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.tasks:
            ...
            self.task_embedding = torch.Tensor(torch.randn(self.task_embedding_dim)).to(
                self.device
            )
            self.task_embedding = nn.Parameter(self.task_embedding)
        self.train_task_embeddings = config.train_task_embeddings
        if self.train_task_embeddings:
            # not activated by default since we project concat of [task_embedding, layer_embedding]
