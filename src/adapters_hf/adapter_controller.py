...
class MetaAdapterController(nn.Module):
    ...
    def apply_pre_layer_norm(self, inputs, task_embeddings):
        ...
        if self.conditional_layer_norm:
            ...
            return torch.nn.functional.layer_norm(
                inputs, (self.input_dim,), weight=weight, bias=bias
            )
        else:
    ...
    def forward(self, task_embedding, inputs):
        ...
        Returns:
            outputs of the adapter layer."""
        z = (
            self.apply_pre_layer_norm(inputs, task_embedding)
            if self.add_layer_norm_before_adapter
            else inputs
        )
...
class MetaLayersAdapterController(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.add_layer_norm_before_adapter = config.add_layer_norm_before_adapter
        self.add_layer_norm_after_adapter = config.add_layer_norm_after_adapter
        self.original_layer_norm_before = config.original_layer_norm_before
        self.original_layer_norm_after = config.original_layer_norm_after
        self.dropout = config.hf_dropout
    ...
    def forward(
        self, hidden_states, adapter_weights, residual_input=None, ln=None, dropout=None
    ):
        if self.original_layer_norm_before:
            inputs = ln(hidden_states + residual_input)
        else:
            inputs = hidden_states + residual_input
        inputs = (
            self.apply_layer_norm(inputs, adapter_weights.pre_norm)
            if self.add_layer_norm_before_adapter
            else inputs
        )
        outputs = self.call_adapter(inputs, adapter_weights)
        if self.add_layer_norm_after_adapter:
            outputs = self.apply_layer_norm(outputs, adapter_weights.post_norm)
        outputs = outputs + hidden_states
        if self.dropout > 0:
            outputs = dropout(outputs)
        if self.original_layer_norm_after:
            outputs = ln(outputs + residual_input)
        else:
            outputs = outputs + residual_input
        return outputs
...
class AutoAdapterController(nn.Module):
    ...
    @classmethod
    def get(cls, config):
        if "efficient_unique_hyper_net" in config:
            config = from_dict(data_class=MetaAdapterConfig, data=config)
            if isinstance(config, MetaAdapterConfig):
                return MetaAdapterController(config)
        elif "conditional_layer_norm" in config:
