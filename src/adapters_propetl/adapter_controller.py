...
def create_score_for_adapter(adapter_model):
    ef_attn_down_scores = nn.Parameter(
        torch.Tensor(adapter_model.down_sampler.weight.size())
    )
    ef_attn_up_scores = nn.Parameter(
        torch.Tensor(adapter_model.up_sampler.weight.size())
    )
...
class AdapterController(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.mask_extreme_mode_combine_method = (
            self.config.mask_extreme_mode_combine_method
        )
        ...
        if self.use_multilingual:
            real_tasks = list(
                set([task_language.split("_")[0] for task_language in self.tasks])
            )
            languages = list(
                set([task_language.split("_")[1] for task_language in self.tasks])
            )
        else:
        ...
        for task in real_tasks:
            down_scores, up_scores = create_score_for_adapter(
                self.adapters["common_adapter"]
            )
            self.mask_scores[task + "_down_mask"] = ParamMask(
                down_scores, self.config.sparsity
            )
            self.mask_scores[task + "_up_mask"] = ParamMask(
                up_scores, self.config.sparsity
            )
        ...
        for language in languages:
            ...
            self.mask_scores[language + "_down_mask"] = ParamMask(
                down_scores, self.config.sparsity
            )
            self.mask_scores[language + "_up_mask"] = ParamMask(
                up_scores, self.config.sparsity
            )

        self.mask_scores = nn.ModuleDict(self.mask_scores)
        if self.mask_extreme_mode:
            self.mask_scores["layer" + "_down_mask"] = ParamMask(
                down_scores, self.config.sparsity
            )
            self.mask_scores["layer" + "_up_mask"] = ParamMask(
                up_scores, self.config.sparsity
            )
    ...
    def forward(self, task, hidden_states, residual_input=None, ln=None):
        ...
        if self.use_multilingual:
            if self.mask_extreme_mode_combine_method == "add":
                down_mask = (
                    self.mask_scores[task.split("_")[0] + "_down_mask"]()
                    + self.mask_scores[task.split("_")[1] + "_down_mask"]()
                    + self.mask_scores["layer" + "_down_mask"]()
                )
                up_mask = (
                    self.mask_scores[task.split("_")[0] + "_up_mask"]()
                    + self.mask_scores[task.split("_")[1] + "_up_mask"]()
                    + self.mask_scores["layer" + "_up_mask"]()
                )
            else:
                down_mask = 1 - (
                    1 - self.mask_scores[task.split("_")[0] + "_down_mask"]()
                ) * (1 - self.mask_scores[task.split("_")[1] + "_down_mask"]()) * (
                    1 - self.mask_scores["layer" + "_down_mask"]()
                )  # Or operation written in this way for easier gradient calculation
                up_mask = 1 - (
                    1 - self.mask_scores[task.split("_")[0] + "_up_mask"]()
                ) * (1 - self.mask_scores[task.split("_")[1] + "_up_mask"]()) * (
                    1 - self.mask_scores["layer" + "_up_mask"]()
                )  # OR operation written in this way for easier gradient calculation
        else:
            if not self.mask_extreme_mode:
                ...
                down_mask = (
                    self.mask_scores[task + "_down_mask"]()
                    * self.mask_scores["layer" + "_down_mask"]()
                )  # AND operation written in this way for easier gradient calculation
                up_mask = (
                    self.mask_scores[task + "_up_mask"]()
                    * self.mask_scores["layer" + "_up_mask"]()
                )  # AND operation written in this way for easier gradient calculation
            elif self.mask_extreme_mode_combine_method == "or":
                down_mask = 1 - (1 - self.mask_scores[task + "_down_mask"]()) * (
                    1 - self.mask_scores["layer" + "_down_mask"]()
                )  # OR  operation written in this way for easier gradient calculation
                up_mask = 1 - (1 - self.mask_scores[task + "_up_mask"]()) * (
                    1 - self.mask_scores["layer" + "_up_mask"]()
                )  # OR operation written in this way for easier gradient calculation
        ...
        # pre-forward for pfeiffer
        if self.config.adapter_config_name == "pfeiffer":
            inputs = ln(hidden_states + residual_input)
            # residual = hidden states
        outputs = adapter(inputs, down_mask, up_mask)
        outputs = outputs + hidden_states
        # post-forward for pfeiffer
        outputs = ln(outputs + residual_input)
        return outputs
...
class AutoAdapterController(nn.Module):
    ...
    @classmethod
    def get(cls, config):
        config = from_dict(data_class=AdapterConfig, data=config)
