RUN_NAME=st-a-soup-include_target-k5-3e-4-GSG
INCLUDE_TARGET=True
TOPK=5
LR=3e-4
...
for TASK in cb copa wsc rte mrpc cola wic boolq stsb multirc sst2 qnli mnli qqp record; do
  for SEED in "${SEEDS[@]}"; do
    ...
    if [ $SEED -gt 2 ] && [ $TASK = "record" -o $TASK = "multirc" -o $TASK = "sst2" -o $TASK = "qnli" -o $TASK = "qqp" -o $TASK = "mnli" ]; then
    ...
    for TRAIN_PCT in 100; do
      ...
      CUDA_VISIBLE_DEVICES=$GPU_ID python ../../run.py \
        ...
        --fp16 \
        --fusion_type soup \
        --soup_sim_file GSG_cos_sims_10000.json \
